...
class ArticleComment extends Service
{
    ...
    {
        $tableArticleComment= Be::newTable('cms_article_comment');
    ...
    {
        return Be::newTable('cms_article_comment')
            ->where($this->createCommentWhere($conditions))
            ->count();
    }
    ...
    public function comment($articleId, $commentBody)
    {
        ...
        $tupleArticleComment = Be::newTuple('cms_article_comment');
    ...
    {
        Be::newTable('cms_article_comment')->where('id', 'in', explode(',', $ids))->update(['block' => 0]);
    }

    public function commentsBlock($ids)
    {
        Be::newTable('cms_article_comment')->where('id', 'in', explode(',', $ids))->update(['block' => 1]);
    }

    public function commentsDelete($ids)
    {
        Be::newTable('cms_article_comment')->where('id', 'in', explode(',', $ids))->delete();
    }
